...
export type PaymentStatus = "pending" | "paid" | "hold_30_days" | "released" | "refunded" | "disputed";
...
export const ownerListings = subscriptionListings.filter((item) => ["r3", "r5", "r6"].includes(item.id));

export const joinedListings = subscriptionListings.filter((item) => ["r1", "r2", "r4", "r5", "r8"].includes(item.id));
