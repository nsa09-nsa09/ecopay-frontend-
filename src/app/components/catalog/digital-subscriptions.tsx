...
// ─── i18n helpers ───
type L = 'ru' | 'kz' | 'en';
const t = (l: L, ru: string, kz: string, en: string) => (l === 'ru' ? ru : l === 'kz' ? kz : en);
    ...
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div
            className="w-11 h-11 rounded-lg flex items-center justify-center shrink-0 text-[15px]"
        ...
        <div className="flex gap-1.5 flex-wrap justify-end">
          ...
          <Chip variant="info">
            <Users size={10} /> {t(lang, 'Семейная группа', 'Отбасы тобы', 'Family group required')}
          </Chip>
        </div>
      </div>
        ...
        {plans.map((p, i) => (
          <button
            ...
            style={{
              ...
              color:
                selectedPlan === i ? 'var(--eco-text-on-primary)' : 'var(--eco-text-secondary)',
      ...
      <Popover
        ...
        trigger={
          ...
          >
            <Info size={12} />{' '}
            {t(lang, 'Полная таблица разбивки', 'Толық бөлу кестесі', 'Full breakdown table')}
          </span>
        }
      >
          ...
          <table className="w-full text-[12px]" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--eco-border)' }}>
                <th className="text-left py-1.5 pr-3" style={{ color: 'var(--eco-text-tertiary)' }}>
                  {t(lang, 'План', 'Жоспар', 'Plan')}
                </th>
                <th
                  className="text-right py-1.5 px-2"
                  style={{ color: 'var(--eco-text-tertiary)' }}
                >
                  {t(lang, 'Цена', 'Бағасы', 'Price')}
                </th>
                {members.map((m) => (
                  <th
                    key={m}
            ...
            <tbody>
              {plans.map((p) => (
                <tr key={p.name} style={{ borderBottom: '1px solid var(--eco-border)' }}>
                  <td className="py-1.5 pr-3" style={{ color: 'var(--eco-text)' }}>
                    {p.name}
                  </td>
  ...
  const applePlans = [
    {
      name: 'Individual',
      price: '$19.95',
      sharing: t(lang, 'Без шеринга', 'Бөліссіз', 'No sharing'),
      seats: 1,
    },
    {
      name: 'Family',
      price: '$25.95',
      sharing: t(lang, 'До 5', '5-ке дейін', 'Up to 5'),
      seats: 5,
    },
    {
      name: 'Premier',
    ...
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div
            className="w-11 h-11 rounded-lg flex items-center justify-center shrink-0 text-[15px]"
            style={{ background: '#00000010', color: '#333' }}
          >
            A
          </div>
          <div>
        ...
        {applePlans.map((p) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-[13px]" style={{ color: 'var(--eco-primary)' }}>
                {p.price}
      ...
      <div className="pt-2 border-t" style={{ borderColor: 'var(--eco-border)' }}>
        <div className="text-[12px] mb-2" style={{ color: 'var(--eco-text-tertiary)' }}>
          iCloud+ Family {t(lang, 'тарифы', 'тарифтері', 'tiers')}
        </div>
        {icloudPlans.map((p) => (
          <div
            key={p.name}
            className="flex items-center justify-between px-3 py-1.5 rounded-lg mb-1"
            style={{ background: 'var(--eco-surface)' }}
          >
            <div className="flex items-center gap-2">
              <span className="text-[13px]" style={{ color: 'var(--eco-text)' }}>
    ...
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div
            className="w-11 h-11 rounded-lg flex items-center justify-center shrink-0 text-[15px]"
            style={{ background: '#FC3F1D18', color: '#FC3F1D' }}
          >
            Я
          </div>
          <div>
            ...
            <div className="text-[12px]" style={{ color: 'var(--eco-text-tertiary)' }}>
              {t(
                lang,
                'Музыка, Кинопоиск, Диск и др.',
                'Музыка, Кинопоиск, Диск т.б.',
                'Music, Kinopoisk, Disk & more',
              )}
            </div>
          </div>
        </div>
      ...
      >
        ...
        <span className="text-[16px]" style={{ color: 'var(--eco-primary)' }}>
          ₸1,999
          <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>
            {' '}
            /{t(lang, 'мес', 'ай', 'mo')}
    ...
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div
            className="w-11 h-11 rounded-lg flex items-center justify-center shrink-0 text-[15px]"
            style={{ background: '#5B57D518', color: '#5B57D5' }}
          >
            Д
          </div>
          <div>
        ...
        <table className="w-full text-[12px]" style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--eco-border)' }}>
              ...
              <th className="text-right py-1.5 px-2" style={{ color: 'var(--eco-text-tertiary)' }}>
                {t(lang, 'Мес', 'Ай', 'Mo')}
              </th>
              <th className="text-right py-1.5 px-2" style={{ color: 'var(--eco-text-tertiary)' }}>
                {t(lang, 'Год (мес)', 'Жыл (ай)', 'Year (mo)')}
              </th>
              <th className="text-right py-1.5 px-2" style={{ color: 'var(--eco-text-tertiary)' }}>
                {t(lang, 'Эконом', 'Үнемдеу', 'Save')}
              </th>
            </tr>
          </thead>
          <tbody>
...
export function DigitalSubscriptionsAvailable() {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 py-12">
      ...
      {/* Disclaimer */}
      <div
        className="mt-6 flex items-start gap-2 px-4 py-3 rounded-lg"
