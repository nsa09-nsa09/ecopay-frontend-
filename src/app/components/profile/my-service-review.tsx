...
import { ApiError, createServiceReview, deleteMyServiceReview, getMyServiceReview, updateMyServiceReview, type ServiceReviewDto } from "../../lib/api";
...
export function MyServiceReviewCard() {
  ...
  useEffect(() => {
    ...
    authorizedRequest((token) => getMyServiceReview(token))
      ...
      .catch(() => { if (!cancelled) setError(t("loadFailedTitle")); })
  ...
  if (!isAuthenticated) {
    return (
      <Card className="flex flex-col gap-3">
        <h3 className="text-[16px]" style={{ color: "var(--eco-text)" }}>{t("serviceReviewMyTitle")}</h3>
        <p className="text-[13px]" style={{ color: "var(--eco-text-secondary)" }}>{t("serviceReviewSignInPrompt")}</p>
  ...
  if (loading) {
    return <Card><span className="text-[13px]" style={{ color: "var(--eco-text-tertiary)" }}>{t("loading")}</span></Card>;
  }

  const submit = async () => {
    if (!text.trim()) { setError(t("serviceReviewTextLabel")); return; }
    setSubmitting(true);
    setError(null);
    setInfo(null);
    try {
  ...
  if (review && !editing) {
    return (
      <Card className="flex flex-col gap-3">
        ...
        {!review.featured && (
          <p className="text-[12px]" style={{ color: "var(--eco-text-tertiary)" }}>{t("serviceReviewModerationNote")}</p>
        )}
        <div className="flex gap-2">
          <Button variant="secondary" size="sm" onClick={() => setEditing(true)}>{t("serviceReviewEdit")}</Button>
          <Button variant="ghost" size="sm" onClick={() => void handleDelete()} loading={submitting}>{t("serviceReviewDelete")}</Button>
        </div>
        {info && <p className="text-[12px]" style={{ color: "var(--eco-positive)" }}>{info}</p>}
      </Card>
    );
  }
    ...
    <Card className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          placeholder={tx(language, "Расскажите о вашем опыте...", "Тәжірибеңіз туралы айтыңыз...", "Tell us about your experience...")}
      ...
      <div className="flex gap-2">
        ...
        {review && (
          <Button variant="ghost" onClick={() => { setEditing(false); setText(review.text); setRating(review.rating); }}>
