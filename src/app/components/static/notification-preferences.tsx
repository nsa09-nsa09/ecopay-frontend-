...
function categoryLabel(category: string, language: Language): string {
  switch (category) {
    case "MEMBERSHIP": return tx(language, "Участие в комнатах", "Бөлмелерге қатысу", "Membership");
    case "ROOM": return tx(language, "Статус комнаты", "Бөлме күйі", "Room status");
    case "PAYMENTS": return tx(language, "Платежи и выплаты", "Төлемдер мен аударымдар", "Payments & payouts");
...
function typeLabel(type: string, language: Language): string {
  switch (type) {
    case "MEMBER_JOINED": return tx(language, "Новая заявка в вашу комнату", "Бөлмеңізге жаңа өтінім", "New applicant to your room");
    case "OWNER_ACCESS_GRANTED": return tx(language, "Владелец предоставил доступ", "Иесі қатынас берді", "Owner granted access");
    case "MEMBER_CONFIRMED": return tx(language, "Участник подтвердил доступ", "Қатысушы қатынасты растады", "Member confirmed access");
    case "MEMBERSHIP_ACTIVATED": return tx(language, "Участие активировано", "Қатысу белсендірілді", "Membership activated");
    case "MEMBERSHIP_REJECTED": return tx(language, "Участие отклонено", "Қатысу қабылданбады", "Membership rejected");
    case "CONFIRMATION_DEADLINE_WARNING": return tx(language, "Напоминание о подтверждении", "Растау туралы еске салу", "Confirmation reminder");
    case "ROOM_ACTIVE": return tx(language, "Комната активирована", "Бөлме белсендірілді", "Room activated");
    case "ROOM_COMPLETED": return tx(language, "Комната завершена", "Бөлме аяқталды", "Room completed");
    case "ROOM_BLOCKED": return tx(language, "Комната заблокирована", "Бөлме бұғатталды", "Room blocked");
    case "ROOM_CANCELLED": return tx(language, "Комната отменена", "Бөлме болдырылмады", "Room cancelled");
    case "PAYMENT_SUCCESS": return tx(language, "Платёж принят", "Төлем қабылданды", "Payment succeeded");
    case "PAYMENT_FAILED": return tx(language, "Платёж не прошёл", "Төлем өтпеді", "Payment failed");
    case "REFUND_ISSUED": return tx(language, "Возврат средств", "Қаражатты қайтару", "Refund issued");
    case "PAYOUT_SENT": return tx(language, "Выплата отправлена", "Аударым жіберілді", "Payout sent");
    case "DISPUTE_OPENED": return tx(language, "Спор открыт", "Дау ашылды", "Dispute opened");
    case "DISPUTE_RESOLVED": return tx(language, "Спор решён", "Дау шешілді", "Dispute resolved");
    case "TICKET_REPLY": return tx(language, "Ответ поддержки", "Қолдау жауабы", "Support reply");
    case "ACCOUNT_BANNED": return tx(language, "Аккаунт заблокирован", "Аккаунт бұғатталды", "Account banned");
    case "ACCOUNT_UNBANNED": return tx(language, "Аккаунт разблокирован", "Аккаунт бұғаттан шығарылды", "Account unbanned");
    default: return type;
  }
}
...
export function NotificationPreferencesPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setPrefs(updated);
      setDirty(false);
      toast.success(tx(language, "Настройки сохранены", "Параметрлер сақталды", "Preferences saved"));
    } catch (err) {
      toast.error(
            ...
            {grouped.map(([category, items]) => (
              <Card key={category}>
                ...
                <div className="hidden sm:flex items-center justify-end gap-6 pb-2 pr-1">
                  <span className="flex items-center gap-1 text-[11px]" style={{ color: "var(--eco-text-tertiary)" }}>
                ...
                <div className="flex flex-col divide-y" style={{ borderColor: "var(--eco-border)" }}>
                  {items.map((p) => (
                    <div key={String(p.type)} className="flex items-center justify-between gap-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--eco-text-secondary)" }}>
                        {typeLabel(String(p.type), language)}
                      </span>
                      <div className="flex items-center gap-6 shrink-0">
                        <Toggle on={p.inApp} onClick={() => setChannel(String(p.type), "inApp", !p.inApp)} />
                        <Toggle
                          on={p.email}
