  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[10px]" style={{ color: 'var(--eco-success-500)' }}>
          {t('govDo')}
        </span>
      </div>
      <div className="p-3 text-[12px]" style={{ color: 'var(--eco-text)' }}>
        {children}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[10px]" style={{ color: 'var(--eco-danger-500)' }}>
          {t('govDont')}
        </span>
      </div>
      <div className="p-3 text-[12px]" style={{ color: 'var(--eco-text)' }}>
...
function TokenRulesSection() {
  ...
  const tokenPairs = [
    {
      semantic: '--eco-primary',
      raw: '#E07A5F',
      desc: 'Brand action color',
      usage: 'Buttons, active tabs, links',
    },
    {
      semantic: '--eco-text',
      raw: '#1A1A2E',
      desc: 'Primary text',
      usage: 'Headings, body, labels',
    },
    {
      semantic: '--eco-text-secondary',
      raw: '#6B7280',
      desc: 'Secondary text',
      usage: 'Descriptions, help text',
    },
    {
      semantic: '--eco-surface',
      raw: '#FFFFFF',
      desc: 'Card/panel bg',
      usage: 'Cards, modals, drawers',
    },
    {
      semantic: '--eco-bg',
      raw: '#F8F8FA',
      desc: 'Page background',
      usage: 'Page bg, table header bg',
    },
    {
      semantic: '--eco-border',
      raw: '#E5E7EB',
      desc: 'Default border',
      usage: 'Cards, dividers, inputs',
    },
    {
      semantic: '--eco-success-500',
      raw: '#22C55E',
      desc: 'Positive status',
      usage: 'Badges, icons, toasts',
    },
    {
      semantic: '--eco-danger-500',
      raw: '#EF4444',
      desc: 'Negative status',
      usage: 'Error, destructive actions',
    },
    {
      semantic: '--eco-warning-500',
      raw: '#F59E0B',
      desc: 'Caution',
      usage: 'Pending, attention badges',
    },
  ];
    ...
    <div>
      ...
      <SC className="mb-5">
        ...
        <div className="flex flex-col gap-2">
          <RuleRow
            rule="Always use semantic token names (--eco-*)"
            why="Enables Eco Coral ↔ Eco Blue theme switching without code changes"
          />
          <RuleRow
            rule="Never hardcode hex values (#E07A5F) in components"
            why="Breaks theming — color will not adapt when user switches theme"
          />
          <RuleRow
            rule="Use -100 / -300 / -500 scale for status colors"
            why="100 = background fill, 300 = border, 500 = text/icon"
          />
          <RuleRow
            rule="Surface hierarchy: --eco-bg → --eco-surface → --eco-surface-raised"
            why="Maintains visual depth — page, card, elevated card"
          />
          <RuleRow
            rule="Spacing: multiples of 8px only (8, 16, 24, 32, 40, 48)"
            why="8pt grid keeps all whitespace harmonious across breakpoints"
          />
        </div>
      </SC>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        <DoBox>
          <code
            className="text-[11px] block rounded px-2 py-1.5 mb-1"
            style={{ background: 'var(--eco-bg)' }}
          >
          ...
          <code
            className="text-[11px] block rounded px-2 py-1.5"
            style={{ background: 'var(--eco-bg)' }}
          >
  ...
  const components = [
    {
      ...
      sizes: ['sm', 'md', 'lg'],
      rule: 'Never create a new button style. Extend via variant prop.',
    },
    {
      name: 'Badge',
      variants: ['success', 'danger', 'warning', 'info', 'default'],
      sizes: ['—'],
      rule: 'All status indicators must use Badge. No ad-hoc colored spans.',
    },
    {
      name: 'Card',
      variants: ['default', 'elevated (surface-raised)'],
      sizes: ['—'],
      rule: 'Wrap all content blocks in Card. Never style raw divs as cards.',
    },
    {
      name: 'Input / Select',
      variants: ['default', 'error', 'disabled'],
      sizes: ['md'],
      rule: 'Use ds-primitives Input/Select. No raw <input> elements.',
    },
    {
      name: 'Toast',
      variants: ['success', 'error', 'warning', 'info'],
      sizes: ['—'],
      rule: 'Toasts only via the toast system. No inline alerts for transient messages.',
    },
    {
      name: 'Modal / Sheet',
      variants: ['center modal', 'bottom sheet (mobile)'],
      sizes: ['sm', 'md', 'lg'],
      rule: 'All overlays through one Modal component. No custom positioned divs.',
    },
  ];
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ background: 'var(--eco-warning-100)', border: '1px solid var(--eco-warning-300)' }}
      >
        <Lightbulb
          size={16}
        ...
        />
        <div>
          <div className="text-[13px]" style={{ color: 'var(--eco-warning-500)' }}>
            Golden Rule: "If it exists in ds-primitives, use it. If it doesn't, add a variant there
            — never in the feature module."
          </div>
          <div className="text-[11px] mt-1" style={{ color: 'var(--eco-text-tertiary)' }}>
            All reusable UI must live in{' '}
            <code className="px-1 rounded" style={{ background: 'var(--eco-neutral-100)' }}>
              ds-primitives.tsx
            </code>
            . Feature pages only compose primitives.
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DoBox>
          <code
            className="text-[11px] block rounded px-2 py-1.5 mb-2"
            style={{ background: 'var(--eco-bg)' }}
          >
        ...
        <DontBox>
          ...
          <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>
            Custom button bypasses the design system
          </span>
        </DontBox>
      </div>
    </div>
...
function CopyRulesSection() {
  ...
  const toneRules = [
    {
      rule: 'Calm, clear, helpful',
      example: "Payment didn't go through. Try another card.",
      bad: 'PAYMENT FAILED!!! ERROR CODE 502.',
    },
    {
      rule: 'Short — 1 sentence max for toasts, 2 for modals',
      example: 'Room joined successfully.',
      bad: 'Congratulations! You have successfully joined the room. You will now be redirected to your rooms page.',
    },
    {
      rule: "No blame — frame as 'what to do next'",
      example: "We couldn't verify your plan. Please re-upload.",
      bad: 'You uploaded an invalid document.',
    },
    {
      rule: 'No jargon or internal codes',
      example: 'Something went wrong. Please try again.',
      bad: 'Error: INTERNAL_SERVER_ERROR at /api/v2/rooms',
    },
  ];
  ...
  const banned = [
    ...
    { phrase: 'Are you sure?', why: "Vague — state what will happen: 'This will leave the room'" },
    {
      ...
      why: "Condescending — use specific time: 'Usually takes 1–2 min'",
    },
  ...
  const lengthLimits = [
    ...
    { element: 'Page heading', max: '50 chars', example: 'My rooms' },
    {
      ...
      max: '120 chars',
      example: 'No rooms yet. Browse the catalog to find a plan that fits.',
    },
    {
      element: 'Error message',
      max: '100 chars',
      example: "Payment didn't go through. Check your card or try another.",
    },
    {
      element: 'Banner text',
      max: '140 chars',
      example: 'Scheduled maintenance Apr 5, 03:00–05:00. Service may be unavailable.',
    },
  ];
  ...
  return (
    <div>
          ...
          {toneRules.map((tr, i) => (
            <div key={i}>
              ...
              <div className="flex gap-3 mt-2 ml-7">
                <DoBox>
                  <span className="text-[11px]">{tr.example}</span>
                </DoBox>
                <DontBox>
                  <span className="text-[11px]">{tr.bad}</span>
                </DontBox>
              </div>
            </div>
          ))}
            ...
            {banned.map((b) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[11px] ml-2" style={{ color: 'var(--eco-text-tertiary)' }}>
                    — {b.why}
                  </span>
                </div>
              </div>
      ...
      <SC>
        ...
        <div className="flex flex-col gap-2">
          <RuleRow
            rule="Never reveal whether an account exists in error messages"
            why="Login: 'Invalid credentials' — never 'User not found' or 'Wrong password'"
          />
          <RuleRow
            rule="Never show technical error details to end users"
            why="Replace stack traces with 'Something went wrong. If this persists, contact support.'"
          />
          <RuleRow
            rule="Rate-limit messages: time-based, no attempt counts"
            why="'Try again in 5 minutes' — not 'You have 2 attempts remaining'"
          />
          <RuleRow
            rule="Payment declines: suggest action, not cause"
            why="'Check card details or try another' — not 'Insufficient funds'"
          />
          <RuleRow
            rule="Fraud holds: neutral, no accusation"
            why="'This action is paused for review' — not 'Suspicious activity detected'"
          />
        </div>
      </SC>
    </div>
...
function IconRulesSection() {
  ...
  const sizeGuide = [
    {
      size: 12,
      use: 'Inline with 11–12px text (badges, timestamps)',
      example: <Clock size={12} />,
    },
    {
      size: 14,
      use: 'Inline with 13–14px text (body, buttons)',
      example: <ChevronRight size={14} />,
    },
    { size: 16, use: 'Section headers, nav icons, input icons', example: <DoorOpen size={16} /> },
    {
      size: 20,
      use: 'Card headers, stat values, feature highlights',
      example: <CreditCard size={20} />,
    },
  ...
  const iconUsage = [
    {
      when: 'Status badge accompaniment',
      icons: 'CheckCircle2, XCircle, AlertTriangle, Info, Clock',
      rule: 'Match icon color to badge variant color',
    },
    {
      when: 'Navigation items',
      icons: 'DoorOpen, CreditCard, Shield, User, Settings',
      rule: '16px, 1.5px stroke, secondary text color',
    },
    {
      when: 'Actions (buttons / row actions)',
      icons: 'Eye, Edit2, Trash2, Download, ExternalLink',
      rule: '14px inside buttons, 13px for table row actions',
    },
    {
      when: 'Empty states',
      icons: 'Inbox, Search, FileX, WifiOff, ShieldOff',
      rule: '32px centered, tertiary color, single icon per state',
    },
    {
      when: 'Form validation',
      icons: 'AlertCircle (error), CheckCircle2 (success)',
      rule: '14px, right-aligned in input, danger/success color',
    },
  ];
    ...
    <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {sizeGuide.map((sg) => (
            ...
            >
              ...
              <span className="text-[16px] tabular-nums" style={{ color: 'var(--eco-text)' }}>
                {sg.size}px
              </span>
              <span
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DoBox>
          ...
          <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>
            14px icon, same color as badge, left-aligned
          </span>
        </DoBox>
        <DontBox>
          <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>
            24px oversized, raw CSS color, mismatched scale
          </span>
        </DontBox>
      </div>
    </div>
...
function StatusRulesSection() {
  ...
  const statuses: {
    ...
    modules: string[];
    userLabel: { ru: string; kz: string; en: string };
    internalKey: string;
  }[] = [
    {
      name: 'ACTIVE',
      badge: 'success',
      modules: ['Rooms', 'Payments', 'Members'],
      userLabel: { ru: 'Активно', kz: 'Белсенді', en: 'Active' },
      internalKey: 'ACTIVE',
    },
    {
      name: 'OPEN',
      badge: 'info',
      modules: ['Rooms', 'Catalog'],
      userLabel: { ru: 'Открыто', kz: 'Ашық', en: 'Open' },
      internalKey: 'OPEN',
    },
    {
      name: 'PENDING',
      badge: 'warning',
      modules: ['Payments', 'Rooms', 'Disputes'],
      userLabel: { ru: 'Ожидание', kz: 'Күту', en: 'Pending' },
      internalKey: 'PENDING',
    },
    {
      ...
      badge: 'warning',
      modules: ['Rooms', 'Admin'],
      userLabel: { ru: 'На проверке', kz: 'Тексеруде', en: 'Under review' },
      internalKey: 'IN_VERIFICATION',
    },
    {
      name: 'COMPLETED',
      badge: 'default',
      modules: ['Payments', 'Disputes'],
      userLabel: { ru: 'Завершено', kz: 'Аяқталды', en: 'Completed' },
      internalKey: 'COMPLETED',
    },
    {
      name: 'BLOCKED',
      badge: 'danger',
      modules: ['Rooms', 'Members', 'Admin'],
      userLabel: { ru: 'Заблокировано', kz: 'Бұғатталған', en: 'Blocked' },
      internalKey: 'BLOCKED',
    },
    {
      name: 'CANCELLED',
      badge: 'danger',
      modules: ['Payments', 'Rooms'],
      userLabel: { ru: 'Отменено', kz: 'Болдырылмады', en: 'Cancelled' },
      internalKey: 'CANCELLED',
    },
    {
      name: 'REFUNDED',
      badge: 'info',
      modules: ['Payments', 'Disputes'],
      userLabel: { ru: 'Возвращено', kz: 'Қайтарылды', en: 'Refunded' },
      internalKey: 'REFUNDED',
    },
    {
      name: 'RESOLVED',
      badge: 'success',
      modules: ['Disputes', 'Support'],
      userLabel: { ru: 'Решено', kz: 'Шешілді', en: 'Resolved' },
      internalKey: 'RESOLVED',
    },
    {
      name: 'EXPIRED',
      badge: 'default',
      modules: ['Rooms', 'Payments'],
      userLabel: { ru: 'Истекло', kz: 'Мерзімі өтті', en: 'Expired' },
      internalKey: 'EXPIRED',
    },
  ];
  ...
  return (
    <div>
      ...
      <SC className="mb-5">
        ...
        <div className="flex flex-col gap-2">
          <RuleRow
            rule="Use the canonical UPPER_SNAKE internal key for data/API"
            why="Prevents 'active', 'Active', 'ACTIVE' inconsistency across modules"
          />
          <RuleRow
            rule="Display user-facing labels via i18n translation, not the key itself"
            why="Users see 'Under review' not 'IN_VERIFICATION'"
          />
          <RuleRow
            rule="Map each status to exactly one Badge variant — no ad-hoc colors"
            why="ACTIVE=success, PENDING=warning, BLOCKED=danger — always"
          />
          <RuleRow
            rule="Never invent new statuses in feature modules"
            why="If a new state is needed, add it to the canonical list + all 3 locales"
          />
          <RuleRow
            rule="Transitions must be documented: OPEN → ACTIVE → COMPLETED"
            why="Prevents impossible states in the UI (e.g. COMPLETED → OPEN)"
          />
        </div>
      </SC>
            ...
            <tbody>
              {statuses.map((s) => (
                <tr key={s.name} style={{ borderBottom: '1px solid var(--eco-border)' }}>
                  ...
                  <td className="px-3 py-2.5">
                    <Badge variant={s.badge}>{s.userLabel.en}</Badge>
                  </td>
                  <td className="px-3 py-2.5" style={{ color: 'var(--eco-text-secondary)' }}>
                    {s.userLabel.ru}
                  </td>
  ...
  return (
    <div>
      ...
      <SC className="mb-5">
        ...
        <div className="flex flex-col gap-2">
          <RuleRow
            rule="Every user-visible string must go through t() — no hardcoded text in JSX"
            why="Enables language switching and ensures all strings are translatable"
          />
          <RuleRow
            rule="Every key must have all 3 locales: { ru, kz, en }"
            why="Missing translations cause runtime fallback to key name (broken UI)"
          />
          <RuleRow
            rule="Key names: camelCase, prefixed by module/context"
            why="Prevents collisions: 'roomTitle' vs 'paymentTitle' instead of just 'title'"
          />
          <RuleRow
            rule="Group keys by section with comment separators"
            why="Keeps the translations file scannable: // ===== Auth ====="
          />
          <RuleRow
            rule="No string concatenation — use template keys with {placeholders}"
            why="Word order differs across RU/KZ/EN — concatenation breaks grammar"
          />
          <RuleRow
            rule="Max 280+ keys: audit quarterly, remove orphans"
            why="Dead keys increase file size and confuse translators"
          />
        </div>
      </SC>
        ...
        <SC>
          ...
          <div className="flex flex-col gap-3">
            {[
              {
                step: '1',
                label: 'Dev adds key with EN value',
                desc: 'Create key in i18n-provider with English text',
              },
              {
                step: '2',
                label: 'RU/KZ placeholders added',
                desc: 'Copy EN as placeholder with [NEEDS_TRANSLATION] prefix',
              },
              {
                step: '3',
                label: 'Native speaker review',
                desc: 'RU + KZ translations reviewed by native speakers',
              },
              {
                step: '4',
                label: 'Truncation test',
                desc: 'Check longest locale (usually KZ) at 390px breakpoint',
              },
              {
                step: '5',
                label: 'Merge to main',
                desc: 'All 3 locales filled + truncation verified',
              },
            ].map((s, i, arr) => (
...
export function GovernanceRulesPage() {
  const { t } = useI18n();
  const [tab, setTab] = useState<'tokens' | 'components' | 'copy' | 'icons' | 'statuses' | 'i18n'>(
    'tokens',
  );
