  ...
  useEffect(() => {
    ...
    async function finalize() {
      try {
        ...
        if (cancelled) return;
        setPhase("failed");
        setMessage(err instanceof ApiError ? err.message : tx(language,
      ...
      <div className="max-w-[640px] mx-auto px-4 sm:px-6 py-8">
        <Card className="flex flex-col items-center text-center gap-4 py-12">
          ...
          <div className="text-[16px]" style={{ color: "var(--eco-text)" }}>
            {tx(language, "Подключаем вашу карту…", "Картаңызды қосудамыз…", "Connecting your card…")}
          </div>
          <div className="text-[13px]" style={{ color: "var(--eco-text-secondary)" }}>
            {tx(language, "Это займёт несколько секунд. Не закрывайте страницу.", "Бұл бірнеше секунд алады. Бетті жаппаңыз.", "This takes a few seconds. Please don't close this page.")}
          </div>
        </Card>
      </div>
    );
  ...
  return (
    <div className="max-w-[640px] mx-auto px-4 sm:px-6 py-8">
      <Card className="flex flex-col items-center text-center gap-5 py-10">
        ...
        <div>
          <h2 className="text-[22px]" style={{ color: "var(--eco-text)" }}>
            {tx(language, "Не удалось подключить карту", "Картаны қосу мүмкін болмады", "Card Not Connected")}
          </h2>
          <p className="text-[14px] mt-2 max-w-sm mx-auto" style={{ color: "var(--eco-text-secondary)" }}>
            {message ?? tx(language, "Попробуйте ещё раз.", "Қайталап көріңіз.", "Please try again.")}
          </p>
        </div>
        <Button variant="primary" size="lg" onClick={() => navigate("/rooms/create")}>
