...
export function Input({ label, error, hint, className = '', ...props }: InputProps) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {hint && !error && (
        <span style={{ color: 'var(--eco-text-tertiary)', fontSize: 12 }}>{hint}</span>
      )}
    </div>
  );
}
        ...
        return (
          <button
            ...
            style={{
              ...
              borderBottom:
                active === id ? '2px solid var(--eco-primary)' : '2px solid transparent',
              marginBottom: -1,
            }}
          >
            {label}
...
export function MemberStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant={memberStatusMap[status] || 'default'}>{statusLabel(status, language)}</Badge>
  );
}
...
export function RoomStatusBadge({ status }: { status: string }) {
  const { language } = useI18n();
  return (
    <Badge variant={roomStatusMap[status] || 'default'}>{statusLabel(status, language)}</Badge>
  );
}
  ...
  return (
    <Card className={`flex flex-col gap-4 relative ${popular ? 'ring-2' : ''}`}>
      ...
      <div>
        ...
        <div className="text-[28px] mt-1" style={{ color: 'var(--eco-text)' }}>
          {price}
        </div>
      </div>
      ...
      <Button variant={popular ? 'primary' : 'secondary'} size="md" className="mt-auto w-full">
        Choose Plan
      </Button>
    </Card>
  );
    ...
    <Card className="flex flex-col gap-3">
      ...
      {items.map((i) => (
        <div key={i.label} className="flex justify-between text-[13px]">
          ...
          <span style={{ color: i.bold ? 'var(--eco-text)' : 'var(--eco-text-secondary)' }}>
            {i.value}
          </span>
        </div>
      ))}
...
export function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      <div className="text-[13px] max-w-xs" style={{ color: 'var(--eco-text-secondary)' }}>
        {description}
      </div>
    </div>
  );
