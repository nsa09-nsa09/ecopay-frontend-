...
export function RoomDetailPage() {
    ...
    return (
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        <Card>
          {tx(
            language,
            'Загрузка деталей комнаты...',
            'Бөлме деректері жүктелуде...',
  ...
  if (loadError || !room) {
    return (
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        <Card>
          {loadError ?? tx(language, 'Комната не найдена.', 'Бөлме табылмады.', 'Room not found.')}
        </Card>
      </div>
    );
  }
  ...
  const handleJoin = async () => {
    ...
    if (!consent) {
      setJoinError(
        tx(language, 'Нужно подтвердить согласие.', 'Келісім қажет.', 'Consent must be accepted.'),
      );
      return;
    }
      ...
      if (err instanceof ApiError) {
        ...
        setJoinError(
          tx(
            ...
            'Өтінімді жіберу мүмкін болмады.',
            'Unable to submit the room request right now.',
          ),
        );
      }
    } finally {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={item.label}>
                  ...
                  <div
                    className="text-[15px]"
                    style={{ color: item.highlight ? 'var(--eco-primary)' : 'var(--eco-text)' }}
                  >
                    {item.value}
                  </div>
                </div>
          ...
          <Card className="flex flex-col gap-3">
            ...
            {[
              ...
              <div key={row.label} className="flex justify-between text-[13px]">
                ...
                <span style={{ color: row.bold ? 'var(--eco-text)' : 'var(--eco-text-secondary)' }}>
                  {row.value}
                </span>
              </div>
            ))}
            <div
              className="border-t pt-3 flex justify-between text-[14px]"
          ...
          <Card className="flex flex-col gap-3">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <ReputationLevelBadge
                    level={summary?.ownerReputationLevel}
            ...
            >
              ...
              {tx(
                ...
                'Деректер шифрланған түрде сақталады.',
                'Data is stored encrypted in the backend.',
              )}
            </div>
          </Card>
      ...
      <Modal
        ...
        title={tx(language, 'Присоединиться к комнате', 'Бөлмеге қосылу', 'Join Room')}
      >
        {!joinDone ? (
            ...
            {joinStep === 1 && (
              <div className="flex flex-col gap-4">
                <Card className="flex flex-col gap-2">
                  ...
                  <div className="flex justify-between text-[13px]">
                    <span style={{ color: 'var(--eco-text-secondary)' }}>
                      {tx(language, 'Провайдер', 'Провайдер', 'Provider')}
                    </span>
