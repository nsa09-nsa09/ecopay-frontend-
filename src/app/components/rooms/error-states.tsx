  ...
  return (
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-12 sm:py-16 text-center">
      ...
      <Card className="text-left mb-6">
        <div className="text-[13px] mb-3" style={{ color: 'var(--eco-text-secondary)' }}>
          {t('roomDetailsLabel')}
        </div>
        {[
    ...
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-12 sm:py-16 text-center">
      ...
      <Card className="text-left mb-6">
        ...
        {[
          ...
          <div key={row.label} className="flex justify-between text-[14px] mb-1.5">
            <span style={{ color: 'var(--eco-text-tertiary)' }}>{row.label}</span>
            <span style={{ color: row.isError ? 'var(--eco-negative)' : 'var(--eco-text)' }}>
              {row.value}
            </span>
          </div>
        ))}
    ...
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-12 sm:py-16 text-center">
      ...
      <Card className="text-left mb-6">
        ...
        {[
          ...
          <div key={row.label} className="flex justify-between text-[14px] mb-1.5">
            <span style={{ color: 'var(--eco-text-tertiary)' }}>{row.label}</span>
            <span style={{ color: row.isStatus ? 'var(--eco-negative)' : 'var(--eco-text)' }}>
              {row.value}
            </span>
          </div>
        ))}
      ...
      <div className="flex flex-col gap-3 items-center">
        <div
          className="p-4 rounded-lg text-[13px] w-full"
