...
export function AdminStoriesPage() {
  ...
  return (
    <AdminLayout>
                    ...
                    return (
                      <tr key={it.id} style={{ borderBottom: '1px solid var(--eco-border)' }}>
                        <td className="py-2 pr-3" style={{ color: 'var(--eco-text)' }}>
                          <div className="flex items-center gap-2">
                            {it.imageUrl ? (
                              <img src={it.imageUrl} alt="" width={36} height={36} className="rounded-full object-cover shrink-0" />
                            ) : (
                              <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ background: it.gradient || DEFAULT_GRADIENT }}>
                                {it.emoji ? <span>{it.emoji}</span> : <ImageIcon size={14} style={{ color: '#fff' }} />}
                              </div>
                            )}
                            <div className="min-w-0">
                              <div className="truncate" style={{ maxWidth: 360 }}>{title || `#${it.id}`}</div>
                              <div className="truncate text-[12px]" style={{ maxWidth: 360, color: 'var(--eco-text-tertiary)' }}>{heading}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-2 pr-3"><StatusBadge status={it.status} t={t} /></td>
                        <td className="py-2 pr-3" style={{ color: 'var(--eco-text-secondary)' }}>{it.sortOrder ?? 0}</td>
                        <td className="py-2 pr-3" style={{ color: 'var(--eco-text-tertiary)' }}>{formatDateTime(it.updatedAt, language)}</td>
                        <td className="py-2 text-right whitespace-nowrap">
                          <Button variant="ghost" size="sm" onClick={() => openEdit(it)}>
                            <Pencil size={13} /> {t('adminNewsEdit')}
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(it)} disabled={deletingId === it.id}>
      ...
      <Modal open={editorOpen} onClose={closeEditor} title={editing ? t('adminStoriesFormEdit') : t('adminStoriesFormCreate')}>
        <div className="flex flex-col gap-4 max-h-[70vh] overflow-y-auto pr-1">
          <Tabs tabs={langTabs} active={activeLang} onChange={(id) => setActiveLang(id as StoryLang)} />

          <FormRow label={t('adminStoriesFieldTitle')}>
            <Input value={currentFields.title} onChange={(e) => setLangField(activeLang, 'title', e.target.value.slice(0, TITLE_MAX))} maxLength={TITLE_MAX} hint={`${currentFields.title.length} / ${TITLE_MAX}`} />
          </FormRow>
          <FormRow label={t('adminStoriesFieldHeading')}>
            <Input value={currentFields.heading} onChange={(e) => setLangField(activeLang, 'heading', e.target.value.slice(0, HEADING_MAX))} maxLength={HEADING_MAX} hint={`${currentFields.heading.length} / ${HEADING_MAX}`} />
          </FormRow>
          <FormRow label={t('adminStoriesFieldBody')}>
            <textarea
              ...
              style={{ background: 'var(--eco-bg)', color: 'var(--eco-text)', border: '1px solid var(--eco-border)', resize: 'vertical' }}
            />
            <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>{currentFields.body.length} / {BODY_MAX}</span>
          </FormRow>
          <FormRow label={t('adminStoriesFieldCtaLabel')}>
            <Input value={currentFields.ctaLabel} onChange={(e) => setLangField(activeLang, 'ctaLabel', e.target.value.slice(0, CTA_MAX))} maxLength={CTA_MAX} />
          </FormRow>
          <FormRow label={t('adminStoriesFieldCtaUrl')}>
            <Input value={form.ctaUrl} onChange={(e) => setForm((prev) => ({ ...prev, ctaUrl: e.target.value.slice(0, URL_MAX) }))} maxLength={URL_MAX} placeholder="/catalog" />
          </FormRow>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <FormRow label={t('adminNewsFieldStatus')}>
              <Select value={form.status} onChange={(e) => setForm((prev) => ({ ...prev, status: e.target.value as StoryStatus }))} options={statusOptions} />
            </FormRow>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <FormRow label={t('adminStoriesFieldEmoji')}>
              <Input value={form.emoji} onChange={(e) => setForm((prev) => ({ ...prev, emoji: e.target.value.slice(0, 16) }))} placeholder="⭐" />
            </FormRow>
            <FormRow label={t('adminStoriesFieldGradient')}>
              <Input value={form.gradient} onChange={(e) => setForm((prev) => ({ ...prev, gradient: e.target.value.slice(0, 255) }))} />
            </FormRow>
          </div>
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center gap-3">
                  {previewUrl ? (
                    <img src={previewUrl} alt="" width={120} height={80} className="rounded-lg object-cover shrink-0" />
                  ) : (
                    <div className="w-[120px] h-[80px] rounded-lg flex items-center justify-center shrink-0" style={{ background: form.gradient || DEFAULT_GRADIENT }}>
                      {form.emoji ? <span className="text-[28px]">{form.emoji}</span> : <ImageIcon size={20} style={{ color: '#fff' }} />}
                    </div>
                  )}
                  <div className="flex flex-col gap-2 flex-1">
                    <input ref={fileInputRef} type="file" accept={ACCEPTED_IMAGE_TYPES.join(',')} className="hidden" onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (file) handlePickFile(file);
                    }} />
                    <div className="flex flex-wrap gap-2">
                      <Button variant="secondary" size="sm" loading={uploadingNow} onClick={() => fileInputRef.current?.click()}>
                        <Upload size={13} /> {previewUrl ? t('adminNewsImageReplace') : t('adminNewsImageUpload')}
                      </Button>
                      {pendingImageFile && (
                      ...
                      {editing?.imageUrl && !pendingImageFile && (
                        <Button variant="ghost" size="sm" onClick={() => void handleRemoveImage()} disabled={uploadingNow}>
                          <X size={13} /> {t('adminNewsImageRemove')}
                        </Button>
                      )}
                    </div>
                    <span className="text-[11px]" style={{ color: 'var(--eco-text-tertiary)' }}>{t('adminStoriesImageHint')}</span>
                  </div>
                </div>
              );
            })()}
          ...
          {editorError && <div className="text-[13px]" style={{ color: 'var(--eco-negative)' }}>{editorError}</div>}

          <div className="flex items-center justify-end gap-2 pt-2">
            <Button variant="ghost" size="sm" onClick={closeEditor} disabled={saving}>{t('cancel')}</Button>
      ...
      <Modal open={confirmDelete !== null} onClose={() => setConfirmDelete(null)} title={t('adminStoriesDeleteConfirm')}>
        {confirmDelete && (
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button variant="destructive" size="sm" loading={deletingId === confirmDelete.id} onClick={() => void handleDelete(confirmDelete)}>
