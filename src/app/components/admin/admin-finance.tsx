...
const PAYMENT_REVIEW_STATUSES = ['UNKNOWN', 'RECONCILING', 'CAPTURE_ANOMALY', 'REQUIRES_REVIEW'];
const REFUND_STATUSES = ['REQUESTED', 'UNDER_REVIEW', 'APPROVED', 'REFUND_PENDING', 'FAILED', 'REQUIRES_REVIEW'];
const PAYOUT_STATUSES = ['ON_HOLD', 'PROCESSING', 'FAILED', 'CLAWBACK_REQUIRED'];
...
function statusVariant(status: string | null): 'default' | 'success' | 'warning' | 'danger' | 'info' {
  ...
  if (['SUCCESS', 'COMPLETED', 'REFUNDED', 'APPROVED'].includes(status)) return 'success';
  if (['FAILED', 'REJECTED', 'CAPTURE_ANOMALY', 'CLAWBACK_REQUIRED'].includes(status)) return 'danger';
  if (['UNKNOWN', 'RECONCILING', 'REQUESTED', 'UNDER_REVIEW', 'REQUIRES_REVIEW'].includes(status)) return 'warning';
  return 'info';
}
...
function RoomRef({ id, title }: { id: number | null; title: string | null }) {
  ...
  return (
    <Link to={`/admin/rooms?selected=${id}`} style={{ color: 'var(--eco-primary)', textDecoration: 'none' }}>
...
function UserRef({ id, name }: { id: number | null; name: string | null }) {
  if (id == null) return <span style={{ color: 'var(--eco-text-tertiary)' }}>-</span>;
  return (
    <Link to={`/admin/users?selected=${id}`} style={{ color: 'var(--eco-primary)', textDecoration: 'none' }}>
      {name ?? `U-${id}`}
    </Link>
  );
}
...
export function AdminFinancePage() {
  ...
  const statusOptions = useMemo(() => {
    const values =
      tab === 'payment-review' ? PAYMENT_REVIEW_STATUSES : tab === 'refunds' ? REFUND_STATUSES : tab === 'payouts' ? PAYOUT_STATUSES : ['DEAD_LETTER'];
    return [{ value: '', label: t('financeFilterAll') }, ...values.map((value) => ({ value, label: value }))];
  }, [tab, t]);
  ...
  return (
    <AdminLayout>
      <div className="max-w-[1280px]">
        <div className="pb-5 mb-6 border-b flex items-center justify-between gap-3 flex-wrap" style={{ borderColor: 'var(--eco-border)' }}>
          <div>
            ...
            <p className="text-[12px] mt-1" style={{ color: 'var(--eco-text-tertiary)' }}>
              Payment review, refunds, payouts and webhook queues from backend operations.
            </p>
          </div>
        ...
        {tab === 'payment-review' ? (
          ...
          <Card className="flex items-start gap-3 py-8">
            <AlertTriangle size={16} style={{ color: 'var(--eco-warning)' }} />
            <div className="text-[13px]" style={{ color: 'var(--eco-text-secondary)' }}>
              This backend operation queue is not exposed in the current frontend API contract, so no financial action buttons are shown.
            </div>
          </Card>
        )}
        ...
        {totalItems > 0 && (
          <div className="flex items-center justify-between mt-4 text-[12px]">
            ...
            <span style={{ color: 'var(--eco-text-tertiary)' }}>{t('pageOf', { page: page + 1, total: totalPages })}</span>
...
function TransactionsTable({ items, language, loading }: { items: FinanceTransactionDto[]; language: 'ru' | 'kz' | 'en'; loading: boolean }) {
        ...
        {items.map((item) => (
          <tr key={item.id}>
            ...
            <Td>
              ...
              <div><UserRef id={item.payerUserId} name={item.payerDisplayName} /></div>
            </Td>
            <Td>
...
function RefundsTable({ items, language, loading }: { items: FinanceRefundDto[]; language: 'ru' | 'kz' | 'en'; loading: boolean }) {
  if (loading && items.length === 0) return <SkeletonRows />;
  if (items.length === 0) return <EmptyOps />;
  return (
    <Table>
      ...
      <tbody>
        {items.map((item) => (
          <tr key={item.id}>
            ...
            <Td>
              <RoomRef id={item.roomId} title={item.roomTitle} />
              <div><UserRef id={item.memberUserId} name={item.memberDisplayName} /></div>
            </Td>
            <Td><PublicId label={item.providerReference ?? '-'} /></Td>
...
function Table({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: 'var(--eco-surface-raised)', border: '1px solid var(--eco-border)' }}>
...
function SkeletonRows() {
  return <Card className="text-[13px] py-10" style={{ color: 'var(--eco-text-tertiary)' }}>Loading operations...</Card>;
}

function EmptyOps() {
  return <Card className="text-center text-[13px] py-10"><span style={{ color: 'var(--eco-text-tertiary)' }}>No operations in this queue.</span></Card>;
}

function Th({ children, className }: { children: ReactNode; className?: string }) {
  return <th className={`px-3 py-2 text-left text-[11px] uppercase ${className ?? ''}`} style={{ color: 'var(--eco-text-tertiary)', letterSpacing: '0.08em', fontWeight: 600 }}>{children}</th>;
}

function Td({ children, className }: { children: ReactNode; className?: string }) {
  return <td className={`px-3 py-2 align-top ${className ?? ''}`} style={{ color: 'var(--eco-text)', borderTop: '1px solid var(--eco-border)' }}>{children}</td>;
}
