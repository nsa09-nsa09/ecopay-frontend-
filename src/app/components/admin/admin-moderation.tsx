      ...
      <div className="max-w-[1100px]">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[13px] mt-1" style={{ color: "var(--eco-text-tertiary)" }}>{t("itemsPendingReview", { count: activeQueue.length })}</p>
          </div>
        </div>
        ...
        {activeQueue.length === 0 ? (
          <Card className="text-center py-12">
            ...
            <div className="text-[13px] mt-1" style={{ color: "var(--eco-text-tertiary)" }}>{t("noItemsPendingModeration")}</div>
          </Card>
        ) : (
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-12 gap-3 px-5 py-2 text-[12px]" style={{ color: "var(--eco-text-tertiary)" }}>
              ...
              <div className="col-span-2">{t("colRiskFlags")}</div>
              <div className="col-span-1">{t("colScore")}</div>
              <div className="col-span-2">{t("colIdentifier")}</div>
              ...
              <Card key={item.id} className="flex flex-col gap-3">
                <div className="grid grid-cols-12 gap-3 items-center">
                  ...
                  <div className="col-span-3">
                    ...
                    <div className="text-[11px]" style={{ color: "var(--eco-text-tertiary)" }}>{item.type === "room" ? t("roomVerification") : t("memberVerification")}</div>
                  </div>
        ...
        <Modal
          ...
          title={actionModal ? `${t(actionModal.action === "CONFIRM" ? "confirm" : actionModal.action === "REJECT" ? "reject" : "requestInfo")} — ${actionModal.item.id}` : ""}
        >
          {actionModal && (
            <div className="flex flex-col gap-4">
              <div className="text-[13px]" style={{ color: "var(--eco-text-secondary)" }}>
                {actionModal.action === "CONFIRM" && t("confirmModerationItem")}
                {actionModal.action === "REJECT" && t("rejectModerationItem")}
                {actionModal.action === "REQUEST_INFO" && t("requestInfoModerationItem")}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  rows={3}
                  placeholder={t("mandatoryActionReason")}
                  value={comment}
              ...
              <div className="text-[11px] flex items-center gap-1" style={{ color: "var(--eco-text-tertiary)" }}>
                <Shield size={11} /> {t("actionRecordedAuditLogs")}
              </div>
              <Button
              ...
              >
                {actionModal.action === "CONFIRM" && t("confirm")}
                {actionModal.action === "REJECT" && t("reject")}
                {actionModal.action === "REQUEST_INFO" && t("sendRequest")}
              </Button>
            </div>
          )}
        </Modal>
        ...
        <Modal open={!!revealTarget && revealedId !== revealTarget} onClose={() => setRevealTarget(null)} title={t("revealIdentifier")}>
          <div className="flex flex-col gap-4">
            <div className="text-[13px]" style={{ color: "var(--eco-text-secondary)" }}>
              {t("revealIdentifierReasonPrompt")}
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px]" style={{ color: "var(--eco-text)" }}>{t("reason")}</label>
              <input
                className="px-3 py-2 rounded-lg outline-none text-[13px]"
                style={{ background: "var(--eco-surface)", border: "1px solid var(--eco-border)", color: "var(--eco-text)" }}
                placeholder={t("reasonPlaceholderModeration")}
                value={revealReason}
                onChange={(e) => setRevealReason(e.target.value)}
              />
            </div>
            <div className="text-[11px] flex items-center gap-1" style={{ color: "var(--eco-text-tertiary)" }}>
              <Shield size={11} /> {t("revealActionLogged")}: {new Date().toISOString().slice(0, 19)}
            </div>
