...
function CategoryFormModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={existing ? t('catalogEdit') : t('catalogCreateCategory')}
    >
      <div className="flex flex-col gap-4">
        <Input
          label={t('catalogFieldName')}
          value={name}
...
function ServiceFormModal({
  ...
  const uploadLogoFor = async (serviceId: number, file: File): Promise<AdminServiceDto | null> => {
    ...
    try {
      const updated = await authorizedRequest((token) =>
        adminUploadServiceLogo(serviceId, file, token),
      );
      return updated;
    } catch (err) {
      setError(formatAdminApiError(err, t));
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={current ? t('catalogEdit') : t('catalogCreateService')}
    >
...
function TariffFormModal({
  ...
  const submit = async () => {
    ...
    if (members < 2) {
      setError('max_members >= 2');
      return;
    }
    if (price <= 0) {
      setError('base_price_total > 0');
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={existing ? t('catalogEdit') : t('catalogCreateTariff')}
    >
      <div className="flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
        ...
        />
        <Input
          label={t('catalogFieldBasePrice')}
          type="number"
          value={basePriceTotal}
          onChange={(e) => setBasePriceTotal(e.target.value)}
        />
        <Input
          label={t('catalogFieldCurrency')}
          value={currency}
          onChange={(e) => setCurrency(e.target.value)}
        />
        <Input
          label={t('catalogFieldConnectionType')}
